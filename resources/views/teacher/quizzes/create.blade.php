        ...
        <div class="card-body p-4">
                    <form action="{{ route('teacher.quizzes.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-lg fw-semibold @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', request('title', '')) }}" placeholder="Enter Quiz Title" required style="border-radius: 10px; font-size: 1.05rem;">
                            @error('title')
                        ...
                        @if(true)
                        <div class="mb-4">
                            <h4 class="mb-3">Questions {{ $difficulty == 'easy' ? '(MCQ)' : ($difficulty == 'medium' ? '(Short Answer)' : '(KBAT)') }}</h4>
